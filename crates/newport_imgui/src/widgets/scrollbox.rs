...
impl Scrollbox {
    pub fn build(self, builder: &mut Builder, contents: impl FnOnce(&mut Builder)) {
        ...
        builder.painter.push_shape(Shape::solid_rect(self.bounds, color.inactive_background, 0.0));
        ...
        let bounds = if retained.last_used > available {
            ...
            let mut scroll_space = match self.direction {
                ...
                Direction::UpToDown|Direction::DownToUp => {
                    bounds.split_right(Self::BAR_SIZE)
                }
            };
            ...
            if is_focused {
                let mouse_move = match self.direction {
                    Direction::LeftToRight => {
                        -builder.input().mouse_move_delta().x
                    },
                    Direction::RightToLeft => {
                        builder.input().mouse_move_delta().x
                    },
                    Direction::UpToDown => {
                        -builder.input().mouse_move_delta().y
                    },
                    Direction::DownToUp => {
                        builder.input().mouse_move_delta().y
                    },
                ...
                retained.target_scroll = (retained.target_scroll + mouse_move * scale).min(max_scroll).max(0.0);
                retained.current_scroll = retained.target_scroll;
            }

            // Mouse scroll input which is in content space
            retained.target_scroll = (retained.target_scroll + builder.input().scroll).min(max_scroll).max(0.0);

            // Interp the current scroll to its target
            retained.current_scroll = retained.current_scroll.interp_to(retained.target_scroll, builder.input().dt, 10.0);
            ...
            builder.painter.push_shape(Shape::solid_rect(scroll_bounds, background_color, 0.0));
        ...
        let layout = builder.layout(Layout::new(bounds, self.direction).with_cursor(-retained.current_scroll), |builder| {
            contents(builder);
        });
