use crate::{asset, engine::Engine, gpu, math, serde, Graphics};

use math::{Rect, Vector2};

use gpu::{BufferUsage, Filter, Format, Layout, MemoryType, Texture, TextureUsage, Wrap};
...
impl FontCollection {
    ...
    pub fn font_at_size(&mut self, size: u32, dpi: f32) -> Option<&Font> {
        ...
        if font.is_none() {
            ...
            let max_dim =
                ((size_metrics.height >> 6) + 1) * (Self::NUM_GLYPHS as f32).sqrt().ceil() as i64;
            let mut tex_width = 1;
            ...
            for i in 0..Self::NUM_GLYPHS {
                ...
                let bmp_width = bmp.width() as i64;
                let bmp_rows = bmp.rows() as i64;
                let bmp_pitch = bmp.pitch() as i64;
