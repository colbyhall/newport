use newport_editor::{
    Alignment, Builder, Direction, Layout, LayoutStyle, Scrollbox, Sizing, Tab, TextEdit,
    TextStyle, SPACING,
};

use crate::{engine::Engine, Game, Named};
...
impl Tab for Outliner {
    ...
    fn build(&mut self, builder: &mut Builder) {
        ...
        builder.layout(Layout::down_to_up(bounds), |builder| {
            ...
            let bounds = builder.layout.push_size(builder.layout.space_left());
            Scrollbox::new("entity_scrollbox", bounds, Direction::UpToDown).build(
                builder,
                |builder| {
