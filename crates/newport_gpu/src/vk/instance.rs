...
impl Instance {
    pub fn new() -> Result<Arc<Self>, InstanceCreateError> {
        ...
        unsafe extern "system" fn debug_callback(message_severity: vk::DebugUtilsMessageSeverityFlagsEXT, _message_types: vk::DebugUtilsMessageTypeFlagsEXT, p_callback_data: *const vk::DebugUtilsMessengerCallbackDataEXT, _p_user_data: *mut ffi::c_void) -> vk::Bool32 {
            let name = ffi::CStr::from_ptr((*p_callback_data).p_message_id_name);
            let message = ffi::CStr::from_ptr((*p_callback_data).p_message);

            let output = format!("{} : {:?} : {:?}", (*p_callback_data).message_id_number, name, message);
        ...
        let debug_utils = unsafe{
            ...
            debug_utils.create_debug_utils_messenger(&debug_create_info, None).unwrap();
